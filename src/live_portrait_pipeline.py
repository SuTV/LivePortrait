        ...
        if args.output_path is not None:
            ...
            if flag_is_driving_video or (flag_is_source_video and not flag_is_driving_video):
                ...
                if I_p_pstbk_lst is not None and len(I_p_pstbk_lst) > 0:
                    images2video(I_p_pstbk_lst, wfp=args.output_path, fps=output_fps)
                else:
                    images2video(I_p_lst, wfp=args.output_path, fps=output_fps)
                ...
                log(f'Animated video: {args.output_path}')
            else:
                if I_p_pstbk_lst is not None and len(I_p_pstbk_lst) > 0:
                    cv2.imwrite(args.output_path, I_p_pstbk_lst[0][..., ::-1])
                else:
                    cv2.imwrite(args.output_path, frames_concatenated[0][..., ::-1])
                # final log
                log(f'Animated image: {args.output_path}')

            return args.output_path, None
        else:
            mkdir(args.output_dir)
            wfp_concat = None
            ######### build the final concatenation result #########
